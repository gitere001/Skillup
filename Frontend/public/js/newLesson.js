...
function autofillReviewDetails() {
    ...
    document.getElementById('review-lesson-description').textContent = description;
...
nextButton.addEventListener('click', () => {
    ...
    if (currentStep === 0) {
        ...
        canProceed = validateField(
            lessonTitle,
            titleMessage,
            lessonTitle.value.trim().length < 5 || lessonTitle.value.trim().length > 70,
   ...
   // Step 1: Validate Course Description
    ...
    // If there's a description, validate its length
    if (lessonDescription.value.trim()) {
        ...
        canProceed = true; // Allow proceeding if there is no input
    }
}
